...
import librosa
import numpy as np
from .config import SILERO_DIR

# Загружаем VAD один раз при импорте
model_vad, utils = torch.hub.load(
    repo_or_dir=str(SILERO_DIR),
    model='silero_vad',
    source='local',
...
def get_speech_segments(audio_path, min_segment_len_s=0.25):
    wav, sr = librosa.load(audio_path, sr=16000)
    wav_tensor = torch.tensor(wav)
    ...
    for seg in speech_ts:
        ...
        end_s = seg["end"] / sr
        if end_s - start_s >= min_segment_len_s:
            valid_segments.append({"start": start_s, "end": end_s, "speaker": "SPEAKER_0"})
